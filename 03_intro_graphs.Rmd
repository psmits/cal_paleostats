...
g3_proj <- bipartite_projection(g3) %>%
  map(., ~ as_tbl_graph(.x, directed = FALSE))
...
canidae <- read_csv(file = url) %>%
  filter(!is.na(formation),            # need formation information
         !str_detect(formation, '[:punct:]')) %>% # get rid of ambiguity
...
- **Degree centrality** is the number of edges associated with that node. The degree distribution is a common focus of statistical analysis.
...
- **PageRank** follows the logic that important nodes are connected to other important nodes.
...
Let's apply these centrality measures to our genus-genus co-occurrence network.
```{r}
...
Community detection algorithms are complicated and highly varied. There are many community detection algorithms which take many different approaches to breaking up the network. 

Here is a brief introduction to a few commonly used algorithms:
...
- **InfoMap** uses mathematics similar to PageRank to estimate the interconnectedness of a graph. This algorithm imagines a random walk on the graph (with teleportation), and seeks to minimize the expected descriptive length of that walk -- by identifying communities, describing the walk takes fewer terms. This algorithm can also return the number of bits necessary to describe the network, a criminally underused value for comparing networks in paleobiology -- see [Sidor et al PNAS 2013](http://www.pnas.org/content/110/20/8129) for an example usage.  
  I encourage you check out [http://www.mapequation.org/](http://www.mapequation.org/) for more information. Also, this is the only algorithm discussed here that is defined to work on bipartite graphs (this is not well known). 

- **Walktrap** is similar to InfoMap in that it envisions a random walker on the graph. Short random walks tend to stay in the same community of a graph. The major conceptual difference between Walktrap and InfoMap is that Walktraop algorithm does not allow the random walker to teleport, so it can get stuck.


There are many more -- tidygraph uses the common predicate `group_*`, so I encourage you to check out some of the other options. In general, I use the InfoMap algorithm -- it is defined by biogeographic networks and has better properties and performance compared to nearly every other algorithm.
...
```{r}
graph_code <- cluster_infomap(canidae_graph)$codelength
