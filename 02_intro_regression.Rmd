...
Let's start with a basic analysis questions: do Bivalves and Brachiopods differ in valve length? Start by looking at the data.
```{r}
d %>%
...
One of the values of the column is designated to be the intercept. In R, the default intercept is the first value of the vector, alphabetically. There are other strategies for encoding categorical variables, but dummy coding is by far the most common -- it helps that it is the R default.
...
An aspect of Bayesian models we've yet to really consider is the *posterior predictive distribution* $p(\tilde{y} | y)$.
